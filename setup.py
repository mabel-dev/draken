...
# Order them with simpler files first to avoid include issues
vector_pyx_files = []
simple_vectors = ['bool_vector.pyx', 'int64_vector.pyx', 'float64_vector.pyx']
for f in simple_vectors:
    if os.path.exists(os.path.join(vectors_dir, f)):
        vector_pyx_files.append(f)

# Then add remaining files
all_pyx_files = sorted([
    os.path.basename(f) for f in glob.glob(os.path.join(vectors_dir, "*.pyx"))
    if os.path.basename(f) not in ("vector.pyx", "vector_implementations.pyx", "arrow_vector.py") + tuple(simple_vectors)
])
vector_pyx_files.extend(all_pyx_files)
